...
export class TokenActionHUD extends Application {
    ...
    return mergeObject(super.defaultOptions, {
        template: '/modules/token-action-hud-relics/templates/template.hbs',
        id: 'token-action-hud-relics',
        classes: [],
        width: 200,
        height: 20,
        left: 150,
    ...
    getData(options = {}) {
        ...
        data.id = 'token-action-hud-relics';
    ...
    activateListeners(html) {
        const tokenactionhud = '#token-action-hud-relics';
        ...
        html.find(repositionIcon).mousedown(ev => {
            ...
            dragElement(document.getElementById('token-action-hud-relics'));
            ...
            function dragElement(elmnt) {
                ...
                function closeDragElement() {
                    ...
                    game.user.update({flags: {'token-action-hud-relics':{ hudPos: {top: yPos, left: xPos}}}})
                }
            }
        });
    ...
    setUserPos() {
        if(!(game.user.data.flags['token-action-hud-relics'] && game.user.data.flags['token-action-hud-relics'].hudPos))
            return;

        let pos = game.user.data.flags['token-action-hud-relics'].hudPos;
        let defaultLeftPos = this.defaultLeftPos;
        let defaultTopPos = this.defaultTopPos;

        return new Promise(resolve => {
            function check() {
                let elmnt = document.getElementById('token-action-hud-relics')
                if (elmnt) {
    ...
    setHoverPos(token) { 
        return new Promise(resolve => {
            function check(token) {
                let elmnt = $('#token-action-hud-relics');
                if (elmnt) {
    ...
    resetPosition() {
        ...
        game.user.update({flags: {'token-action-hud-relics': {hudPos: { top: 80, left: 150 }}}})
        this.update();
    }
