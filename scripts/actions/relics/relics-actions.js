...
export class ActionHandler5e extends ActionHandler {
    ...
    _getActiveEquipment(equipment) {
        const activationTypes = Object.keys(game.relics.config.abilityActivationTypes).filter(at => at !== 'none');
    ...
    _categoriseSpells(actor, tokenId, spells) {
        ...
        let dispose = spells.reduce(function (dispose, s) {
            ...
            const prepType = game.relics.config.spellPreparationModes[prep];
    ...
    _addSpellInfo(s, spell) {
        ...
        spell.info3 = '';
        if (c?.vocal)
            spell.info1 += this.i18n('RELICS.ComponentVerbal').charAt(0).toUpperCase();

        if (c?.somatic)
            spell.info1 += this.i18n('RELICS.ComponentSomatic').charAt(0).toUpperCase();

        if (c?.material)
            spell.info1 += this.i18n('RELICS.ComponentMaterial').charAt(0).toUpperCase();

        if (c?.concentration)
            spell.info2 += this.i18n('RELICS.Concentration').charAt(0).toUpperCase();

        if (c?.ritual)
            spell.info3 += this.i18n('RELICS.Ritual').charAt(0).toUpperCase();
    }
    ...
    _buildSkillsCategory(token) {
        ...
        let skillsActions = Object.entries(skills).map(e => {
            try {
                    let skillId = e[0];
                    let name = abbr ? skillId : game.relics.config.skills[skillId];
    ...
    _addMultiSkills(list, tokenId) {
        ...
        let skillsActions = Object.entries(game.relics.config.skills).map(e => {
     ...
     _getAbilityList(tokenId, abilities, categoryId, categoryName, macroType) {
        ...
        let actions = Object.entries(game.relics.config.abilities).map(e => {
    ...
    _filterNonpreparedSpells(spells) {
        const nonpreparableSpells = Object.keys(game.relics.config.spellPreparationModes).filter(p => p != 'prepared');
        let result = spells;
