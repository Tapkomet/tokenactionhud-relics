export class FilterSubcategory {
    ...
    async updateFlag(categoryId) {
        ...
        await game.user.setFlag('token-action-hud-relics', `categories.${categoryId}.subcategories.${this.id}`, contents);
    }

    async unsetFlag(categoryId) {
        if (categoryId)
            await game.user.setFlag('token-action-hud-relics', `categories.${categoryId}.subcategories`, {[`-=${this.id}`]: null})
    }
