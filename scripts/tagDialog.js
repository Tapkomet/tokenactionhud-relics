export class TagDialog extends Dialog {
    ...
    static showDialog(suggestions, selected, indexChoice, title, hbsData, submitFunc) {
        ...
        let template = Handlebars.compile('{{> modules/token-action-hud-relics/templates/tagdialog.hbs}}');
        ...
        let d = new TagDialog({
            ...
            buttons: {
                ...
                callback: (async (html) => {
                    ...
                    let index = html.find('select[id="token-action-hud-relics-index"]');
                    let indexValue;
                    if (index.length > 0)
    ...
    static _prepareHook(choices, selection, indexChoice) {
        Hooks.once('renderTagDialog', (app, html, options) => {
            ...
            var $index = html.find('select[id="token-action-hud-relics-index"]')
            if ($index.length > 0) {
                if (indexChoice)
            ...
            var $tagFilter = html.find('input[class="token-action-hud-relics-taginput"]');
